...
Seed gym: `pro-gym-montreal`, `primaryColor: #2E7D32`, `secondaryColor: #000000`.

## Multi-tenant gym

Admin session carries `gymId`. APIs scope mutations to that gym. Member home loads slug `pro-gym-montreal` (fallback: first gym).
...
| Workout + history | localStorage keys `pro-gym-montreal-workout:v1:<slug>` and `pro-gym-montreal-workout-history:v1:<slug>` |

...
Members never log in. `/` is the in-product gym home for **Pro Gym Saint-Léonard**.
...
3. Friendly URLs: `/m/pro-gym-montreal/lat-pulldown`.
...
Keyed by gym slug (`pro-gym-montreal`). No member account.

| Key | Contents |
|-----|----------|
| `pro-gym-montreal-workout:v1:<slug>` | In-progress session JSON |
| `pro-gym-montreal-workout-history:v1:<slug>` | Array of saved workouts (newest first) |
